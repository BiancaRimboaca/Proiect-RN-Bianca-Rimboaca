...
|-----------------------|----------------------|-----------------------------|
| Prevenirea supraîncălzirii unui motor BLDC utilizat într-un braț robotic | Predicția temperaturii la pasul următor (T_next) folosind rețea neuronală → declanșare warning sau shutdown înainte de depășirea pragului critic | Neural Network + State Machine |
| Monitorizarea continuă a comportamentului termic în funcție de curent și turație | Simulare termică + inferență RN în timp real → afișare temperatură reală vs prezisă în interfața web | Data Acquisition + RN + Streamlit UI |
| Reducerea riscului de avarie termică și creșterea duratei de viață a motorului | Detectare automată depășire prag (70°C warning / 80°C shutdown) și oprire automată simulată | State Machine + Logică protecție |
...
Total observații finale: 15000  
Observații originale: 15000 (100%)
...
Datele au fost generate prin simularea discretă a modelului termic al unui motor BLDC,
folosind ecuația:

dT/dt = (P_loss - h(ω)(T - T_amb)) / C

unde:
- P_loss = k_i2 * I² + k_w * ω
- h(ω) = h0 + h1 * ω

Simularea a fost realizată pe 5000 pași temporali (dt = 0.1s), generând
temperatura următorului pas T_next în funcție de:
T_amb, omega, I și T_prev.

Semnalele I și ω au fost generate prin random walk pentru a simula
variații realiste de sarcină în aplicații robotice.

Locația codului: src/data_acquisition/generate_data.py  
Locația datelor: data/raw/bldc_raw.csv  

Dovezi:
- Grafic distribuții temperatură: docs/temperature_distribution.png
- Statistici descriptive: docs/data_statistics.csv
...
Am ales arhitectura de monitorizare continuă deoarece proiectul
vizează prevenirea supraîncălzirii unui motor BLDC utilizat
în aplicații robotice (ex: braț robotic industrial).

Motorul trebuie monitorizat permanent deoarece:
- curentul și turația variază în funcție de sarcină
- temperatura poate crește progresiv până la avarie

Stările principale sunt:

1. IDLE – Sistemul așteaptă comanda de pornire.
2. RUN – Se generează date (I, ω, T_amb), se calculează T_real și T_pred.
3. WARNING – Temperatura depășește pragul de 70°C.
4. SHUTDOWN – Temperatura depășește 80°C → oprire automată.
5. STOP – Sistem oprit manual sau automat.

Tranzițiile critice sunt:
- RUN → WARNING: când T_pred ≥ 70°C
- RUN → SHUTDOWN: când T_real sau T_pred ≥ 80°C
- ANY → STOP: comandă manuală

Starea SHUTDOWN este esențială deoarece în aplicații reale
supraîncălzirea poate duce la:
- degradarea izolației înfășurărilor
- scurtcircuit
- distrugerea motorului

Bucla de feedback funcționează astfel:
Temperatura prezisă este comparată cu pragurile,
iar sistemul decide continuarea funcționării sau oprirea.

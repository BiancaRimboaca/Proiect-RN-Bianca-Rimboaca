...
def step_temperature(
    ...
    # Pierderi (simplificat): P_loss = k_i2 * I^2 + k_w * omega
    P_loss = k_i2 * (I ** 2) + k_w * omega

    # Convecție / răcire dependentă de turație: h(omega) = h0 + h1 * omega
    h = h0 + h1 * omega
    cooling = h * (T_prev - T_amb)

    # Dinamică 1st order: dT/dt = (P_loss - cooling) / C
...
def generate_signals(
    ...
    # Semnale variabile în timp (nu random total): folosim random-walk ușor

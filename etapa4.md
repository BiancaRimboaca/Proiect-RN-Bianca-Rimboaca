...
|---------------------------|--------------------------------|--------------------------------|
| Prevenirea supraîncălzirii și defectării premature a mototrului electric în sarcină | Estimarea temperaturii în timp real pe baza curentului și turației cu o eroare MSE < 10% | RN + Modului Predictie |
| Reducerea timpilor de nefuncționare neplanificați (downtime) în liniile de producție | Monitorizare continuă "virtual sensor" (fără senzori fizici scumpi), reducând opriri neplanificate cu 20–30% | Pipe Date + Interfață utilizator |
| Optimizarea regimurilor de lucru pentru eficiență energetică | Ajustarea sarcinii motorului bazată pe predicția termică rapidă (< 1 secundă timp de răspuns) | Modul Antrenare + Modul Predicție |
...
**Descriere detaliată:**
Am generat setul de date integral (100%) folosind un simulator fizic implementat în Python (`generate_data.py`), care modelează comportamentul termic al unui motor electric. Am utilizat distribuții uniforme pentru a genera parametrii de intrare în intervale industriale standard: -Turație (500–3000 RPM), 
-Curent (10–80 A),
-Tensiune (220–400 V).

Pentru variabila țintă (**Temperatura**), am implementat un model matematic neliniar care simulează încălzirea prin efect Joule ($I^2 \cdot R$) și răcirea dependentă de ventilație (turație), peste care am suprapus un zgomot Gaussian ($\mu=0, \sigma=3$) pentru a replica erorile de măsurare ale senzorilor reali și fluctuațiile de mediu. Această abordare asigură robustețea modelului neuronal la date zgomotoase, simulând condiții reale de funcționare.

**Locația codului:** `src/generate_data.py`
**Locația datelor:** `data/raw/motor_data_raw.csv`

**Dovezi:**
- Fisierul `data/raw/motor_data_raw.csv` conține cele 1000 de înregistrări brute
- Distribuția datelor (verificată în etapa de analiză) confirmă corelația fizică între curent și temperatură
- Graficele comparative din etapa EDA demonstrează acoperirea uniformă a regimurilor de funcționare (mers în gol vs. sarcină)
```
...
```
IDLE → START_MONITORING → ACQUIRE_DATA (Turație, Curent, Tensiune) → VALIDATE_INPUT (verificare valori fizice) → 
PREPROCESS (Normalizare StandardScaler) → RN_INFERENCE → CHECK_LIMITS (Prag Critic 90°C)→ 
  ├─ [Normal] → LOG_RESULT → UPDATE_DASHBOARD → ACQUIRE_DATA (loop)
  └─ [Supraîncălzire] → TRIGGER_ALERT → NOTIFY_OPERATOR → LOG_INCIDENT → 
                  ACQUIRE_DATA (loop)
       ↓ [User stop / Emergency]
     SAFE_SHUTDOWN → STOP
```
